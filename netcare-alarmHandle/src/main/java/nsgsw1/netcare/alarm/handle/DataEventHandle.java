...
@Component
@Scope("prototype")
public class DataEventHandle extends AlarmHandleImpl {

	private final static Logger logger = LoggerFactory
			.getLogger(DataEventHandle.class);

	public DataEventHandle() {
	...
	private void handleMe() {
		if (logger.isDebugEnabled())
			logger.debug("DataEventHandle preProcess.objectType is ME");
	...
	private void handlePort() {
		if (logger.isDebugEnabled())
			logger.debug("DataEventHandle preProcess.objectType is PORT");
		QueryCircuitModel sncModel = circuitService.findSncModelByDn(
